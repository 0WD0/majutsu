...
Majutsu uses Emacs' =display-buffer= system for window placement. Customize
=majutsu-display-buffer-function= (Magit-style strategies) or
=display-buffer-alist= to control where log, diff, and blob buffers appear.
