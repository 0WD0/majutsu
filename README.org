...
- [[file:docs/majutsu.org][User Manual]] - Complete guide to all features, including the template DSL's
  helper-local ~:bind-self~, syntax-level ~majutsu-template-defspecial~
  lowering, anonymous ~:lambda~ / ~:|arg|~ values, basic semantic typing for
  ...
  ~Unknown~ unless explicitly annotated), and Dash-style higher-order helpers
  built on native lambda support.
