...
6. In custom ~majutsu-log-template-<field>~ values, ~heading~ can use real newlines; other log modules should use ="\x1f"= for rendered newlines

** Useful Options

- ~majutsu-jj-diffstat-columns~ controls forced width for =jj diff --stat= in narrow terminals/TRAMP (set to =nil= to inherit terminal width; increase for more full path visibility).
- ~majutsu-jj-environment~ prepends custom =KEY=VALUE= entries to every jj subprocess via the process layer.
