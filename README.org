...
2. Navigate: vanilla bindings use ~n~ / ~p~ for next/previous revision; evil bindings use ~C-j~ / ~C-k~ or ~gj~ / ~gk~ for section navigation. In log buffers, ~[~ / ~]~ jump to visible parents/children
3. Act: ~RET~ to visit, ~?~ for help
4. In blob buffers, press ~e~ to enter editable mode; save applies via =jj diffedit= (~i~ with evil bindings also enters editable mode)
