...
- *Workspace support* (~jj workspace~) with list/add/forget/visit commands (Magit-style ~Z~ menu)
...
- ~Z~ / ~%~ - Open the workspace transient
...
columns:
- ~g~, ~c~, ~l~, ~N~, ~s~, ~r~, ~b~, ~G~, ~Z~, ~D~ — same commands as above
...
*** Workspace (~Z~)
- ~l~ - List workspaces in a dedicated buffer
- ~v~ / ~RET~ - Visit a workspace (opens a Majutsu log buffer rooted at that workspace)
- ~a~ - Add a workspace (~jj workspace add~) and then visit it
- ~f~ - Forget workspaces (~jj workspace forget~)
- ~n~ - Rename current workspace
- ~u~ - Update stale working copy (~jj workspace update-stale~)

If ~Visit~ can't auto-locate the workspace directory, customize
~majutsu-workspace-search-directories~ to add parent directories to search.
