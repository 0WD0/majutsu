...
- [[file:docs/majutsu.org][User Manual]] - Complete guide to all features, including the template DSL's
  ...
  types, option-contained method chaining, literal-aware checks for upstream
  ~StringLiteral~ parameters, metadata-driven higher-order typing, the
  ...
  built on native lambda support, and legacy binder-based forms such as
  ~:map~ / ~:filter~ / ~:any~ / ~:all~ / ~:map-join~ as syntax-only
  compatibility sugar.
