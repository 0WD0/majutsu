...
(defun majutsu-forge--insert-pullreq-commits-around (fn pullreq &rest args)
  "Call FN for PULLREQ unless Majutsu should hide Forge's commit list."
...
(defun majutsu-forge--save-binding (map key)
  "Remember MAP's current binding for KEY before changing it."
...
(defun majutsu-forge--set-key (map key command)
  ...
  (when (and (boundp map) (keymapp (symbol-value map)) (fboundp command))
    (majutsu-forge--save-binding map key)
...
(defun majutsu-forge--restore-bindings ()
  ...
  (pcase-dolist (`((,map ,key) ,binding) majutsu-forge--saved-bindings)
...
(defun majutsu-forge--after-forge-refresh (&optional buffer)
  "Refresh Majutsu buffers after `forge-refresh-buffer'."
