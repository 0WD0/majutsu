...
(ert-deftest majutsu-gerrit-account-candidate-data/uses-suggest-by-default ()
  "Account completion should use the optimized suggest endpoint by default."
  (let (seen)
    (cl-letf (((symbol-function 'majutsu-gerrit-rest-current-spec)
      ...
      (let* ((default-directory "/repo")
             (payload (majutsu-gerrit-account-candidate-data "gerrit" "ali"))
             (annotations (plist-get payload :annotations)))
...
(ert-deftest majutsu-gerrit-account-candidate-data/can-use-query-strategy ()
  "When the strategy is `query', use the query endpoint unconditionally."
  (let (seen)
    (cl-letf (((symbol-function 'majutsu-gerrit-rest-current-spec)
               (lambda (&rest args)
                 (setq seen (plist-put seen :spec-args args))
              ...
              ((symbol-function 'majutsu-gerrit-rest-account-suggest)
               (lambda (&rest args)
                 (setq seen (plist-put seen :suggest-args args))
                 nil))
              ((symbol-function 'majutsu-gerrit-rest-account-query-with-details)
               (lambda (&rest args)
      ...
      (let* ((majutsu-gerrit-account-completion-strategy 'query)
             ...
             (payload (majutsu-gerrit-account-candidate-data "gerrit" "ali"))
             (annotations (plist-get payload :annotations)))
        (should-not (plist-get seen :suggest-args))
