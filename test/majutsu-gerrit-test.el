...
(ert-deftest majutsu-gerrit-account-candidate-data/prefers-rest-suggestions ()
  (cl-letf (((symbol-function 'majutsu-gerrit--rest-account-candidate-data)
             (lambda (&rest _args) '(:candidates ("alice@example.com"))))
            ((symbol-function 'majutsu-gerrit--gerrit-account-candidate-data)
             (lambda (&rest _args) '(:candidates ("admin@example.com"))))
            ((symbol-function 'majutsu-gerrit--egerrit-account-candidate-data)
             (lambda (&rest _args) '(:candidates ("bob@example.com")))))
    (should (equal (majutsu-gerrit-account-candidate-data nil nil "ali")
                   '(:candidates ("alice@example.com"))))))

(ert-deftest majutsu-gerrit-account-candidate-data/falls-back-to-external-clients ()
  (cl-letf (((symbol-function 'majutsu-gerrit--rest-account-candidate-data)
             (lambda (&rest _args) nil))
            ((symbol-function 'majutsu-gerrit--gerrit-account-candidate-data)
             (lambda (&rest _args) nil))
            ((symbol-function 'majutsu-gerrit--egerrit-account-candidate-data)
             (lambda (&rest _args) '(:candidates ("bob@example.com")))))
    (should (equal (majutsu-gerrit-account-candidate-data nil nil "bob")
                   '(:candidates ("bob@example.com"))))))

(ert-deftest majutsu-gerrit--rest-account-candidate-data/uses-account-suggest ()
  (let (seen)
              ...
              ((symbol-function 'majutsu-gerrit-rest-account-suggest)
               (lambda (&rest args)
                 ...
                 '(((_account_id . 1001)
                    ...
                    (username . "alice")
                    (email . "alice@example.com"))))))
      (let* ((payload (majutsu-gerrit--rest-account-candidate-data
                       "gerrit" "/repo" "ali"))
             (annotations (plist-get payload :annotations)))
        (should (equal (plist-get seen :spec-args) '("gerrit" "/repo")))
        (should (equal (plist-get seen :suggest-args)
                       '("ali" 15 (:host "review" :scheme "https"))))
        (should (equal (plist-get payload :candidates)
                       '("alice@example.com")))
        (should (equal (gethash "alice@example.com" annotations)
                       "Alice Example @alice #1001"))))))

(ert-deftest majutsu-gerrit-account-completion-table/uses-query-result-candidates ()
  (let (seen)
    (cl-letf (((symbol-function 'majutsu-gerrit-account-candidate-data)
               (lambda (&rest args)
                 ...
                 '(:category majutsu-gerrit-account
                   :candidates ("alice@example.com")))))
      (let ((table (majutsu-gerrit-account-completion-table "gerrit" "/repo")))
        (should (equal (all-completions "Alice" table)
                       '("alice@example.com")))
        (should (equal seen '("gerrit" "/repo" "Alice")))))))
  ...
  (let (seen)
    (cl-letf (((symbol-function 'majutsu--toplevel-safe)
      ...
      (should (equal (majutsu-gerrit-upload--read-cc "CC" nil nil)
                     '("bob@example.com")))
      (should (equal seen
                     '("CC" nil majutsu-gerrit-cc-history
                       "gerrit" "/repo"))))))
