...
(ert-deftest majutsu-tag-delete/interactive-confirms-selection ()
  (let (called confirmed)
    (cl-letf (((symbol-function 'majutsu-tag--read-patterns)
               (lambda (&rest _args) '("v1.0")))
              ...
              ((symbol-function 'message)
               (lambda (&rest _args) nil)))
      (call-interactively #'majutsu-tag-delete)
      (should (equal confirmed '(tag-delete "Delete tag(s) v1.0? ")))
      (should (equal (seq-remove #'null (flatten-tree called))
                     '("tag" "delete" "v1.0"))))))
