...
- Command: majutsu-evolog :: View =jj evolog= with its graph preserved and each entry parsed through Majutsu's graph-entry template protocol. Entries render as compact =jj= evolog sections; full change, commit, and optional operation IDs are kept as hidden metadata for future actions. =RET= and =d= currently use the ordinary parent-mode behavior rather than an evolog-specific patch action.
