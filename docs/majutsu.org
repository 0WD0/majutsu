...
- Command: majutsu-evolog :: View =jj evolog --no-graph= as structured sections for a revision/change. Press =RET= or =d= on an entry to open that entry's native =inter_diff()= patch in =majutsu-evolog-show-mode=.
