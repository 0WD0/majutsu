...
- Key: X (majutsu-op-transient) :: Open the operation dispatcher.
- Command: majutsu-op-log :: View Jujutsu's operation history using structured =jj op log --no-graph= output while preserving jj colors.
  Each operation is rendered as an expanded multiline section with id, user, workspace, time/duration, description, and the first tag line such as =args:=. Press =RET= or =d= on an operation to show details.
