...
*** Log Display
Each revision in the log is a section. The visible heading content is produced by fields assigned to the ~heading~ module in ~majutsu-log-commit-columns~ (for example, change ID, bookmarks, tags, description, author, and timestamp). Collapsed by default, you can expand a revision to see foldable ~body~ module content (for example, long descriptions).
...
- *body/metadata modules*: stay in the tail payload; encode logical newlines as =\x1f= and decode them in postprocessing.

Parsing is strictly sequential and positional (not key/value based):

1. Detect =\x1dS= and lock the entry indent column.
2. Collect heading lines until =\x1dB= is found.
3. Parse =B -> M -> E= payload segments in order on the final heading line (no extra physical line breaks between these module markers).
...
- ~:module~ :: One of ~heading~, ~body~, ~metadata~.
...
Majutsu ensures metadata field ~id~ is always available for stable section identity even if omitted from user configuration.

**** Module Semantics
- ~heading~ :: Visible section heading content (can span physical lines).
...
Author and timestamp now belong in the ~heading~ module; the ~right-margin~ module has been removed.
Move fields between visual areas by changing ~:module~, and adjust display behavior with ~:face~ and ~:post~.
...
(majutsu-tpl [:if [:root] "(root)" [:method [:raw "self"] :commit_id :short]])
#+end_src
...
keywords become method calls on =self=:
#+begin_src emacs-lisp
...
** List Operations
List methods can be written in three equivalent styles:
...
(majutsu-tpl [:method [:raw "refs"] :map [:lambda [c] [:method 'c :description]]])
; => "refs.map(|c| c.description())"

;; Dash-style explicit lambda
(majutsu-tpl [:-map [:lambda [c] [:method 'c :description]] [:raw "refs"]])
; => "refs.map(|c| c.description())"
...
Prefer direct lambdas and =map + join= composition in new code; =:map= and
=:map-join= remain available as compatibility sugar. The historical
=[:map collection var body]= form now lowers to =[:method collection :map
[:lambda [var] body]]=, these binder-style forms are now syntax-only
compatibility sugar rather than value-level Elisp helpers, =:-map= remains the
value-level explicit-lambda helper, and the =:--map= family is syntax sugar
layered on top of the same native lambda support.

Upstream jj itself centers the higher-order story around expressions such as
+=collection.map(|x| body)+. The =:--map= family is therefore a Majutsu DSL
surface convenience that lowers to explicit lambda-based core forms; it is not
an independent runtime/value primitive.
...
When a helper omits its body, =majutsu-template-defun= now defaults to a
simple direct wrapper around the same jj callable name:
#+begin_src emacs-lisp
...
Syntax-level sugar can be defined separately using
=majutsu-template-defspecial=, which receives raw forms and lowers them to
more primitive template syntax. Majutsu now uses this layer for operators such
as =:|arg|=, the anaphoric =:--map= family, and the legacy
=[:map collection var body]= / =:map-join= compatibility forms; the matching
old Elisp helpers were removed so these binder-based constructs live only in
syntax space:
#+begin_src emacs-lisp
...
(majutsu-template-compile '[:canonical-log-id] 'Commit)
...
Ordinary helpers can still rebind the implicit =self= inside their body using
=:bind-self=:
#+begin_src emacs-lisp
...
Majutsu also propagates some core result types through the normalized AST. For
...
preserve the element type. There is no separate =ListTemplate= semantic type;
mapped output is just another list type in the DSL. This is what makes chains
like =[:method [:raw "self" :Commit] :parents :first :description]=
type-aware in the DSL.

Builtin metadata is also kept close to current upstream =jj/docs/templates.md=
for methods such as =files()= / =conflicted_files()=, =normal_target()=,
=workspace_name()=, =since()=, and =root()=. This now also includes upstream's
string-like method dispatch for =RefSymbol= values, so chains like
=[:method [:raw "bookmark" :CommitRef] :name :len]= type-check and lower like
ordinary string method calls. For helper argument checks, =RefSymbol= is also
accepted where a concrete =String= is expected, and builtin helpers now carry
more accurate metadata such as upstream-style conditional handling for =if()=.
That means the conditional result itself behaves like an =Any= expression and is
validated at the consumption site for capabilities such as =Template= or
=Serialize=, rather than being assigned an eager fixed joined type. Majutsu
also lets method chains on =(:option T)= continue through the contained value
type, mirroring upstream's option-method behavior.

Reusable lambda bodies can be defined directly as ordinary helpers returning
...
#+begin_src emacs-lisp
(majutsu-tpl [:lambda [c] [:method 'c :description]])
; => "|c| c.description()"

(majutsu-tpl [:|c| [:method 'c :description]])
; => "|c| c.description()"

(majutsu-tpl [:method [:raw "self" :Commit] :parents :map [:|c| [:description]]])
; => "self.parents().map(|c| c.description())"

(majutsu-tpl [:call [:|c| [:description]] [:raw "item" :Commit]])
; => "item.description()"

(majutsu-tpl [[:lambda [c] [:method 'c :description]] [:raw "item"]])
; => "item.description()"
#+end_src

The shorthand =[:|c| BODY]= is equivalent to =[:lambda [c] BODY]=.

Lambda parameters are lexical variables and can act as a deferred implicit
=self= for bare keyword dispatch. A surface lambda such as
+=[:lambda [c] [:description]]= is therefore kept generic first and can later be
specialized from a typed call argument or a higher-order container element. In
those cases =[:description]= becomes equivalent to =[:method 'c :description]=.
This deferred behavior is limited to lambda parameters; Majutsu no longer uses
unknown non-lambda receivers as a general bare-keyword fallback. If you need
the outer template =self= explicitly, write it out as an explicit object in
=:method= forms.

This is a deliberate Majutsu DSL adaptation. In upstream jj template syntax,
lambda parameters and =self= are distinct names; Majutsu rebinds single-
argument lambdas to implicit =self= for authoring convenience while still
lowering to ordinary jj lambda/method forms.
...
Majutsu now distinguishes between broad =Any= expression placeholders and the
narrower printable =Template= capability. Use =Any= for helper parameters that
merely shuttle expressions around at DSL-expansion time (for example method
objects, raw snippets, or higher-order bodies). Use =Template= when the helper
semantically expects content that can actually be rendered/concatenated.

A few upstream categories are still intentionally simplified in Majutsu's
checker. Upstream =StringLiteral= parameters are now checked in a first,
literal-aware way: direct string literal nodes are accepted, but Majutsu does
not yet attempt full parse-time literal tracking through arbitrary helper calls
or constant folding. Upstream =AnyList= style results are still normalized into
ordinary container refs rather than separate semantic runtime classes.
