...
*** Copying From Graph Entry Buffers
Majutsu now provides both default copy filtering and explicit semantic copy commands for graph-entry buffers such as ~majutsu-log-mode~ and ~majutsu-evolog-mode~. These commands are available through the dispatcher copy submenu and via ~M-x~.
...
- Key: ? w f (majutsu-graph-entry-copy-field) :: Copy the rendered field value at point.
- Key: ? w F (majutsu-graph-entry-copy-entry-field) :: Choose any canonical field stored on the current entry, including hidden metadata that is not visible in the current layout.
- Key: ? w h (majutsu-graph-entry-copy-commit-id) :: Copy the current entry's commit hash from hidden metadata.
- Key: ? w m (majutsu-graph-entry-copy-module) :: Copy the rendered visible module at point (~heading~, ~tail~, or ~body~), without graph-prefix or tail-spacer decoration.

This keeps ordinary Emacs copying predictable while still offering precise graph-entry-aware copy operations when you want semantic rather than purely visual text.
...
- Command: majutsu-evolog :: View =jj evolog= with its graph preserved and each entry parsed through Majutsu's graph-entry template protocol. Entries render as compact =jj= evolog sections; full change, commit, and optional operation IDs are kept as hidden metadata and are available through the shared ~? w~ graph-entry copy submenu. =RET= and =d= currently use the ordinary parent-mode behavior rather than an evolog-specific patch action.
