...
* Manipulating
** Creating Changes
- ~N~ (Emacs), ~O~ (Evil log mode) :: Create a new change on top of the revision at point (DWIM).
- ~n~ (Emacs), ~o~ (Evil) :: Open the New transient to specify options.
- ~A~ (Emacs/Evil log mode) :: Create a new change after the revision at point.
...
** Describing Changes
- ~d~ (~majutsu-describe~) :: Edit the description of the revision at point. Supports =--ignore-immutable= flag for immutable revisions.
