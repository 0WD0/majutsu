...
*** Buffer Display
Majutsu uses Emacs' =display-buffer= system for showing log, diff, and blob buffers. This means window history is preserved and =quit-window= restores the previous view reliably. You can customize buffer placement via =majutsu-display-buffer-function= (Magit-style strategies) or standard =display-buffer= rules such as =display-buffer-alist=.
The default quit behavior uses =majutsu-bury-buffer-function=, which mirrors Magit's window lifecycle handling.
