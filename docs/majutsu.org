...
*** Log Display
Each revision in the log is a section. The visible heading content is produced by fields assigned to the ~heading~ module in ~majutsu-log-commit-columns~ (for example, change ID, bookmarks, tags, and description). Collapsed by default, you can expand a revision to see foldable ~body~ module content (for example, long descriptions).
...
- *body/right-margin/metadata modules*: stay in the tail payload; encode logical newlines as =\x1f= and decode them in postprocessing.
...
3. Parse =B -> R -> M -> E= payload segments in order on the final heading line (no extra physical line breaks between these module markers).
...
- ~right-margin~ :: Rendered into right margin overlays.
...
If ~:face~ is omitted, Majutsu falls back to ~majutsu-log-field-faces~.
...
- Omitted (uses default postprocessors)
...
~CTX~ includes at least ~:field~ and ~:module~.

Default behavior includes decoding =\x1f= into =\n=.
...
** Display Options
- ~majutsu-log-commit-columns~ :: Ordered log field layout for log rendering. For protocol details, module semantics, and schema examples, see /Inspecting -> Log Buffer/.
- ~majutsu-log-field-faces~ :: Fallback face policy when a ~majutsu-log-commit-columns~ entry omits ~:face~.
