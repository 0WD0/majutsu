...
keywords because it better matches the implicit-self model. This deferred
behavior is limited to lambda parameters; Majutsu no longer uses unknown
non-lambda receivers as a general bare-keyword fallback. If you need an outer
receiver explicitly inside a nested lambda, use =[:self N]=.
