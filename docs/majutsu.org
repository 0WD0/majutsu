...
- ~E~ (=majutsu-ediff-edit=) :: Run =jj diffedit= with Emacs as
  ...
  and opens the right-side temp file for manual edit. Press ~C-c C-c~ to finish
  (or ~C-c C-k~ to abort).

Conflict resolution is now a separate command:
- ~m~ in dispatcher (=majutsu-ediff-resolve=) :: Resolve conflicted files.
  Uses =jj resolve= (with-editor based file editing) for 2-sided conflicts, and falls
  back to =jj diffedit= for conflicts with more than 2 sides.
