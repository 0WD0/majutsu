...
- Key: f (majutsu-workspace-forget) :: Forget a workspace. With =-t= enabled from the workspace transient, also move its directory to the system trash after =jj workspace forget= succeeds. Majutsu refuses to trash a workspace root that owns the shared =.jj/repo= store.
