...
- ~e~ / ~C-x C-q~ :: Enter editable blob mode (wdired-style). Save to apply through =jj diffedit= and update the revision.
  In Evil normal state, ~i~ also enters editable blob mode and switches to insert.
  In editable mode, ~C-x C-q~ runs ~majutsu-blob-edit-exit~ (prompting to save or abort when modified).
...
- Recursive trace: running ~b~ again on an annotated chunk jumps to its parent
  revision (when available) and re-annotates there.
- Styles: press ~c~ to cycle heading/highlight/line styles
...
- ~E~ (=majutsu-ediff-edit=) :: Run =jj diffedit= with Emacs as
  ...
  and opens the right-side temp file for manual edit. Saving finishes the
  session and updates the revision (use ~C-c C-k~ to abort). Set
  =majutsu-edit-finish-on-save= to nil to require ~C-c C-c~ instead.

Conflict resolution is now a separate command:
- ~m~ in dispatcher (=majutsu-ediff-resolve=) :: Resolve conflicted files.
  Uses =jj resolve= with 3-way Ediff (merge tool: =$left=, =$base=, =$right=)
  for 2-sided conflicts, and falls back to =jj diffedit= for conflicts with
  more than 2 sides.
  In a =jj-commit= section, conflicted files are listed for that revision.
...
- In a diff hunk, use ~C-c ^ RET~ (=majutsu-diff-resolve-conflict=).
