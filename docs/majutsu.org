...
[:lambda [var] body]]=, =:-map= remains the value-level explicit-lambda helper,
and the =:--map= family is syntax sugar layered on top of the same native
lambda support.
...
+=[:map collection var body]= / =:map-join= compatibility forms:
#+begin_src emacs-lisp
