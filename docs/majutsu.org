...
- Command: majutsu-op-log :: View Jujutsu's operation history using structured =jj op log --no-graph= output while preserving jj colors.
  Each operation is rendered as an expanded multiline section with id, user, workspace, time/duration, description, and the first attribute line such as =args:=. Read-only operation views pass =--at-op=@ --ignore-working-copy= to avoid snapshotting while browsing. Press =d= on an operation to open the operation-diff transient prefilled from point/selection, =u= to restore to it, and =r= to revert it.
