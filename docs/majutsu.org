...
=workspace_name()=, =since()=, and =root()=. Majutsu also lets method chains on
=(:option T)= continue through the contained value type, mirroring upstream's
...
(majutsu-tpl [:call [:|c| [:description]] [:raw "item"]])
; => "item.description()"
...
Lambda parameters are lexical variables and also become the current implicit
=self= for bare keyword dispatch. Inside lambda bodies, =[:description]= is
therefore equivalent to =[:method 'c :description]= when the parameter is
named =c=. If you need the outer template =self= explicitly, write it out as
an explicit object in =:method= forms.
