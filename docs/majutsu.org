...
- Split inherits exactly one resolvable =--revisions= source as =--revision=;
  it never passes a diff's arbitrary =--from= / =--to= range to =jj split=.
- Squash turns =--revisions= into =--from= sources and retains those sources
  when you choose an explicit destination.  An arbitrary =--from= / =--to=
  tree diff is not treated as a Squash source.
...
2. *Tool Invocation*: On the Majutsu patch-selection path, Jujutsu's
   ...
   An explicitly requested user =--tool= instead takes the jj diff-editor
