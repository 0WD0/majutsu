...
- Command: majutsu-op-log :: View Jujutsu's operation history using structured =jj op log --no-graph= output while preserving jj colors. Press =RET= or =d= on an operation to show details.
- Command: majutsu-op-show :: Show metadata, description/tags, and sectionized repository changes for one operation. Changed commit and ref target lines keep full commit/change IDs internally; press =RET= or =d= on a line to open its ordinary commit diff.
