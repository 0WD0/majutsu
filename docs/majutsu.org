...
- Command: majutsu-op-show :: Show metadata, description/tags, and sectionized repository changes for one operation.
  Sections are expanded by default. Changed commit and ref target lines keep full commit/change IDs internally; press =RET= for the point-specific default action. Commit lines behave like normal =jj-commit= sections, while =d= keeps the ordinary Majutsu diff command from the parent mode.
