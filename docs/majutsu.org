...
- Key: J / K :: Swap the selected revision with its unique target parent or child.
...
Majutsu loads target revisions plus external parent/child context. The first backend only exposes constrained graph-neighbor swaps and keep/abandon actions, matching what it can safely replay with =jj rebase= and =jj abandon=.
