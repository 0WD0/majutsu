...
unknown non-lambda receivers as a general bare-keyword fallback. If you need an
outer receiver explicitly inside a nested lambda, use =[:self N]=.
...
semantically expects content that can actually be rendered or concatenated.
...
mapped output is just another list type in the DSL.

Builtin metadata is kept close to current upstream =jj/docs/templates.md= for
methods such as =files()= / =conflicted_files()=, =normal_target()=,
=workspace_name()=, =since()=, and =root()=. This also includes upstream's
...
accepted where a concrete =String= is expected.

Conditional results now follow upstream's =if() -> Any= model. The result of
=if()= is validated at the consumption site for capabilities such as
=Template= or =Serialize= instead of being assigned an eager fixed joined type.
Majutsu also lets method chains on =(:option T)= continue through the contained
value type, mirroring upstream's option-method behavior.
