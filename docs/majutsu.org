...
** Display Options
- ~majutsu-log-commit-columns~ :: Ordered log field layout. Each entry supports ~:field~, ~:module~, ~:face~, and optional ~:post~. Modules are ~heading~, ~body~, ~right-margin~, ~metadata~.
- ~majutsu-log-field-faces~ :: Fallback face policy when a ~majutsu-log-commit-columns~ entry omits ~:face~.
- ~majutsu-log-template-<field>~ variables :: ~heading~ is the only module with physical newlines. For other modules, use ="\x1f"= inside a field template where you want a rendered newline after parsing.
