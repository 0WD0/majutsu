...
- ~E~ (=majutsu-ediff-edit=) :: Run =jj diffedit= with Emacs as
  =ui.diff-editor=, using Ediff directories as the editing UI.
- ~m~ (=majutsu-ediff-resolve=) :: Resolve conflicted files. Git-style 2-way
  conflicts use =smerge-ediff=; JJ-style or multi-side conflicts fall back to
  =majutsu-conflict= workflow.
- ~M~ (=majutsu-ediff-resolve-with-conflict=) :: Open the file and enter the
  conflict-mode workflow directly.
...
- ~b~ :: Keep conflict base (rebase destination).
