...
- Command: majutsu-evolog :: View =jj evolog= with its graph preserved and each entry parsed through Majutsu's graph-record template protocol. Press =RET= or =d= on an entry to open that entry's native =inter_diff()= patch in =majutsu-evolog-show-mode=.
