...
[:lambda [var] body]]=, and public Dash-style helpers such as =:-map= and
=:--map= are ordinary convenience helpers layered on top of the same native
lambda support.
...
(majutsu-tpl [:call [:|c| [:method 'c :description]] [:raw "item"]])
; => "item.description()"
...
Lambda parameters are lexical variables, not a rebinding of =self=. Inside the
body, bare keywords such as =[:description]= still refer to the outer =self=
unless a helper explicitly uses =:bind-self=.
