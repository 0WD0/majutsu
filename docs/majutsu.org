...
- Key: W :: Save stable push options as defaults for this jj repository. Majutsu remembers sync policy such as =--remote=, =--all=, =--tracked=, =--deleted=, =--allow-empty-description=, and =--allow-private=, but does not remember one-off targets like =--bookmark=, =--change=, =--revisions=, =--named=, or =--dry-run=.
Runs: =jj git push=
...
- Key: a (majutsu-git-remote-add-transient) :: Add a new Git remote. The add transient exposes =--fetch-tags= and =--push-url=.
- Key: d (majutsu-git-remote-remove) :: Remove a remote.
- Key: r (majutsu-git-remote-rename) :: Rename a remote.
- Key: u (majutsu-git-remote-set-url-transient) :: Set remote URLs. With no option, Majutsu prompts for the fetch URL; use =--fetch= and/or =--push= to set them explicitly.
