...
- Command: majutsu-op-diff-transient / majutsu-op-diff :: Compare repository changes by operation, =--from=, or =--to= in a sectionized changed commit/ref view.
  Changed lines keep full commit/change IDs internally. Press =RET= or =v= on a changed line to open its evolution log; =d= remains the ordinary Majutsu diff command. Operation metadata plus this diff view replace a separate operation-show buffer.
