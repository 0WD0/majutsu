...
› 我想象中的参数传递的方式是直接让它们共用一个节点，

• 没错，参数共享同一个节点就是我想做的方式：

  - 在宏展开/语法糖阶段都保持结构化节点，不去 copy-tree，而是直接把 majutsu-template-node 结构放进父节点的 args 里。这样多个调用引用
    同一个参数时，本质上就是同一个 cons。
  - 类型推断 pass 也只会对这个节点跑一次，把 type/props 写回结构体；其它引用读到的都是同一块数据。
  - 渲染阶段准备一个 hash（用 eq 或节点里的唯一 id 作 key），第一次编译节点时落地成字符串并缓存，后续只要再次遇到同一个节点，就直接拿
    cached raw。这样既保证“只编译一次”，也避免重复逃逸。

  也就是说，参数的“传递”其实就是共享节点引用，整个管线都会以这一份 AST 为准。
