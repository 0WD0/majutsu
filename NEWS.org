...
- Add explicit semantic copy commands for log buffers:
  ~majutsu-copy-section-value~, ~majutsu-log-copy-field~, and
...
- Add a ~tail~ log module for right-aligned auxiliary fields on the anchor
  ...
  default while copying the tail itself preserves it. Rendered log spans now
  carry field/column/entry/decoration properties so explicit copy commands can
  target section values, individual fields, or whole visible modules.
