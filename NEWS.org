...
- Rows/Bookmarks: Extend ~majutsu-row~ to parse push/pop tree streams and render
  ...
  protocol.
- Completion: Stop mutating ~marginalia-annotators~ and rely on standard
  completion metadata directly for optional Marginalia annotations, while also
  dropping the leftover dependence on Marginalia's private alignment text
  property, replacing borrowed Marginalia styling with Majutsu-owned
  completion faces and separator settings, and no longer fabricating generic
  fallback suffixes for candidates without structured metadata.
...
- Completion: Standardize history-aware readers and completion categories for
  ...
  includes other workspace working-copy refs in =<workspace>@= form. Structured
  completion payloads are now exposed through standard completion metadata, and
  the optional Marginalia integration uses those builtin annotations to show
  richer bookmark, tag, remote, workspace, revision, and file details.
