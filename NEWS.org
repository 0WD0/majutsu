...
- Completion: Standardize history-aware readers and completion categories for
  ...
  older jj versions. Native revset prompts now complete continuously inside
  expressions such as =main | = and =trunk()..ma=. The new
  ~majutsu-jj-revset-completion-mode~ option controls whether revset prompts
  prefer expression-correct =basic= completion or inherit global completion
  styles for setups such as Orderless annotation filtering. The optional
  Marginalia integration now shows richer bookmark, tag, remote, workspace,
  revision, and file annotations based on structured completion payloads.
