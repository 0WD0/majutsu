...
- Operations: Add ~majutsu-op-transient~ on ~X~, top-level ~majutsu-evolog~ on
  ~v~, dedicated ~op log~ / ~op diff~ transients, a color-preserving structured
  multiline ~jj op log~ view with current-operation markers, direct
  restore/revert point actions, point-prefilled operation-diff transients from
  ~d~, expanded sectionized ~majutsu-op-diff~ buffers,
  graph-preserving ~majutsu-evolog~ view, and ~op
  restore~ / ~op revert~ actions.  Read-only operation/evolution views use
  =--at-op=@ --ignore-working-copy= to avoid snapshotting while browsing.
- Bug references: Integrate ~bug-reference-mode~ with Majutsu log and JJ
  description buffers, including JJ Git remote based auto-setup.
- Arrange: Add initial ~majutsu-arrange-mode~ for editing =jj arrange=-style
  graph rewrite plans with target/context loading, keep/abandon marking,
  graph-neighbor swaps, and a conservative CLI replay backend.
- Bookmarks: Add ~majutsu-bookmark-advance~ for =jj bookmark advance=,
  plus ~majutsu-bookmark-advance-to~ for the explicit =--to= form and
  ~majutsu-bookmark-advance-patterns~ for explicit bookmark
  name/pattern selection with the configured default target.
- Commands: Add ~majutsu-revert~ transient for =jj revert= with source
  revision selection and destination placement options (=--onto= /
  =--insert-after= / =--insert-before=).
- Commands: Add ~majutsu-metaedit~ transient for =jj metaedit=. It targets
  a single revision from the region, point, or =@= fallback and exposes
  metadata edit/update flags.
- Commands: Add ~majutsu-simplify-parents~ transient with both =--source=
  and =--revision= options.
- Commands: Add Magit-style ad-hoc runners: ~majutsu-command~,
  ~majutsu-jj-command~, ~majutsu-jj-command-topdir~,
  ~majutsu-shell-command~, and ~majutsu-shell-command-topdir~.
  ~majutsu-mode-map~ now binds =!= to ~majutsu-command~, =Q= and =:= to
  ~majutsu-jj-command~.  Native Evil integration binds ~|~ to
  ~majutsu-jj-command~.
- Transients: Revset arguments now use jj-native completion in command
  context, so menus such as diff/rebase/restore/new complete the same
  revision candidates as the jj CLI.
- File inspection: Add ~majutsu-find-file-other-window~ and
  ~majutsu-find-file-other-frame~ for opening blob buffers in another window
  or frame.
- Log: Add visible DAG relation navigation. ~[~ jumps to a parent revision and
  ~]~ jumps to a child revision, prompting when multiple visible targets exist.
- Log: Add semantic copy commands under the log dispatcher copy submenu for
  section values, rendered fields, hidden entry fields, commit hashes, and
  visible modules.
- JJ descriptions: Add save/history/search/diff commands for description
  buffers, including ~majutsu-jjdescription-save-message~,
  ~majutsu-jjdescription-prev-message~,
  ~majutsu-jjdescription-next-message~,
  ~majutsu-jjdescription-search-message-backward~,
  ~majutsu-jjdescription-search-message-forward~, and
  ~majutsu-jjdescription-show-diff~.
- Template DSL: Add first-class anonymous functions, concise
  ~[:|arg| body]~ syntax, compile-time lambda application, and
  ~majutsu-template-defspecial~.
- Gerrit: Add a ~jj gerrit upload~ transient under the Git dispatcher, with
  revision selection/toggle overlays, remote/branch targeting, review metadata,
  state, notification, advanced upload flags, and repository-local defaults for
  stable upload policy.

** Changed

- Keybindings: Duplicate commands are no longer bound directly to ~y~ / ~Y~
  in Majutsu buffers.  Use the dispatcher entries ~? y~
  (~majutsu-duplicate~) and ~? Y~ (~majutsu-duplicate-dwim~) instead.  This
  keeps Evil's yank keys free while leaving duplicate available from both log
  and diff buffers.
- Evil: The Majutsu dispatcher now shows the keys that Evil users actually
  press for remapped top-level commands, including ~x~, ~L~, ~_~, ~*~, ~u~,
  ~C-r~, and ~`~.
- Rows/Bookmarks: Extend ~majutsu-row~ to parse push/pop tree streams and render
  nested Magit sections, add first-class layout node ~:role~ metadata for
  Magit section class/value/entry-id decisions, use simple field/function
  references for role entry specs, align row↔section lookup with Magit's
  section identity model, and migrate bookmark lists to that shared row
  protocol.
- Bookmarks: The bookmark list buffer now uses typed =jj bookmark list -T=
  machine records with customizable Majutsu template display fragments for
  bookmark headings, commit summaries, and structured conflicted-target heading
  columns, while assigning precise section values such as =name@remote= for tracked remote
  rows. The emitted bookmark tree is declared through
  ~majutsu-bookmark-list-layout~, using layout-level ~:schema~ defaults plus
  uniform node-local ~:columns~ for regular refs and conflicted target rows.
- Transients: Add a generic repository-local defaults layer keyed by jj's
  secure repo =config-id=. Log, diff, and Git sync transients can save current
  arguments as defaults for only the current repository, while global defaults
  continue to use Transient's normal storage.
- Squash: Remove the user-facing =-r/--revision= shortcut from the transient.
  Majutsu now treats =--from= as the canonical squash source, including
  inherited diff =--revisions= revsets, and passes an explicit =--into= default
  when no destination is selected.  With an existing source selection, the
  current log section is preferred as the destination when it is outside the
  source set; otherwise Majutsu uses the source set's external parent.  When a
  destination is selected without a source, the source defaults to =@=.
- Remotes: Add shared remote readers and completion payloads for exact remote
  names, new remote names, and remote name patterns. Git and bookmark commands
  now use the same remote history, completion category, remote-list parsing,
  and remote-at-point defaulting.
- Git: Remote management now follows the upstream =remote add [OPTIONS]
  REMOTE URL= shape more closely: adding a remote prompts for a new remote name
  and URL/path positional, rejects existing names, and exposes =--fetch-tags=
  and =--push-url=. =remote remove=, =rename=, and =set-url= now read existing
  remotes more carefully and default to the remote at point in remote-list
  buffers. =remote set-url= supports explicit =--fetch= and =--push= URL
  updates.
- Git: Expose simple upstream options that were missing from the transients:
  repeated fetch remotes/branches, repeated clone branches, and repeated
  =git push --option= values. Clone sources now use the same URL/path handling
  as remote URLs.
- Log: The log transient's =-r/--revision= filter is now a normal transient
  argument stored with the rest of the log arguments. It uses Majutsu's standard
  revset expression reader like diff =-r=, prefills the current filter for
  editing, and clears the filter when submitted empty.
- Completion: Standardize history-aware readers and completion categories for
  revsets, bookmark/tag names and patterns, remote names/patterns,
  workspace names, and repo-relative file paths using structured completion
  payloads. Native revset prompts now use
  minibuffer completion at point, so expression arguments such as =main | = and
  =trunk()..ma= keep jj's native semantics while still allowing users to move
  the candidate list into a minibuffer for annotation-based filtering. Native
  jj revset completion now includes other workspace working-copy refs in
  =<workspace>@= form, and structured completion payloads are exposed through
  standard completion metadata for richer bookmark, tag, remote, workspace,
  revision, and file details.
- Completion: Expose standard completion metadata directly, use
  Majutsu-owned completion faces and separator settings for structured
  suffixes, and no longer fabricate generic fallback suffixes for candidates
  without structured metadata.
- Thing-at-point: Rework JJ at-point accessors in a more Magit-like style.
  Revset readers and commands such as ~majutsu-edit-changeset~,
  ~majutsu-new-dwim~, and ~majutsu-diff-dwim~ now prefer a literal
  revision/ref under point where appropriate, while bookmark/tag readers
  default to exact refs at point before falling back to revision-based
  bookmark patterns.
- Workspace: Workspace lists now show resolved roots when available and
  ...
  this now uses one structured =jj workspace list -T= parse pipeline.
- Log folding: Preserve display-only graph prefixes when folding and
  unfolding log commits, including via Evil / evil-collection section
  commands.
- Log: Add the ~tail~ module for right-aligned auxiliary fields and use
  display-only prefixes for graph/body indentation so ordinary copying and
  cursor movement see semantic content.
- Log: Store field/column/entry/decoration metadata on rendered spans and
  always keep hidden ~id~, ~commit-id~, and ~parent-ids~ transport data
  available for copy and relation-navigation commands.
- Log: Repeated fields can now render differently in different modules.
  Majutsu decodes transport values before running each column's ~:post~
  handlers.
- Template DSL: Rework implicit ~self~ around explicit/bindable receivers and
  owner-bound extension definitions; local log helpers now use that lowering
  model.
- Template DSL: Lower higher-order forms through the core lambda/method
  machinery and add semantic type propagation for helper arguments, method
  chains, list elements, and deferred lambda specialization.
- Template DSL: Align builtin helper, method, conversion, and operator
  metadata with upstream =jj=, including option/list receivers, ~RefSymbol~,
  literal-only parameters, ~if()~, and helper return types.
- Absorb: =--into= should accept =<REVSETS>= (repeatable)

** Removed

- File inspection: Remove ~majutsu-find-file-at-point~; use
  ~majutsu-find-file~, which still defaults revset/path from context.
- Diff: Remove the dedicated ~v~ binding from diff file sections, following
  Magit's split between explicit file commands and diff-specific visits.
- Log: Remove ~majutsu-log-field-faces~; configure field faces directly in
  ~majutsu-log-commit-layout~ using each field's ~:face~ property.
...
- Ediff: Quitting ~majutsu-ediff-resolve~ without edits no longer writes a
  resolved result, so conflicts stay unresolved.
- Ediff: Edited merge sessions now ask before saving, and partial
  resolutions keep remaining conflicts as jj-parseable git-style markers.
- Evil: Restore evil-collection-style diff-context bindings in
  =majutsu-diff-mode=: ~+~ adds context, ~=~ removes context, and =~=
  resets to the default.
- Filesets: Split, Squash, Restore, and Absorb now emit transient fileset
  selections after option arguments, fixing commands such as
  ~jj squash -- FILE --from=REV~ being parsed as invalid filesets.
