...
- Operations: Add ~majutsu-op-transient~ on ~X~, dedicated ~op log~ / ~op diff~
  ...
  sectionized ~majutsu-op-show~ / ~majutsu-op-diff~ buffers, ~majutsu-evolog~ /
  ~majutsu-evolog-show~ evolution views, and ~op restore~ / ~op revert~ actions.
...
- JJ helpers: Add ~majutsu-jj-buffer-string~ for callers that need full
  plain-text command output instead of line-oriented ~majutsu-jj-lines~.
