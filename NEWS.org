...
- Operations: Add ~majutsu-op-transient~ on ~X~, a color-preserving structured
  multiline ~jj op log~ view, and expanded sectionized ~majutsu-op-show~ buffers
  for operation metadata, tags, changed commits/refs, and per-line commit diff
  actions. Native Evil integration covers the operation log/show modes.
