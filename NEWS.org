...
- Add Magit-style JJ description history commands and bindings:
  ~majutsu-jjdescription-save-message~, ~majutsu-jjdescription-prev-message~,
...
- Add first-class anonymous functions via ~[:lambda [arg] body]~ and concise
  ...
  ~:map~, ~:filter~, ~:any~, ~:all~, ~:map-join~, and the anaphoric ~:--map~
  family now lower through the same core lambda/method forms instead of
  relying on separate helper implementations.
...
- Restore evil-collection-style diff-context bindings in =majutsu-diff-mode=:
  ~+~ adds context, ~=~ removes context, and =~= resets to the default.
