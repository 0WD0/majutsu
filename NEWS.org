...
- Operations: Add ~majutsu-op-transient~ on ~X~, dedicated ~op log~ / ~op diff~
  ...
  sectionized ~majutsu-op-show~ / ~majutsu-op-diff~ buffers, and ~op restore~ /
  ~op revert~ actions. Read-only operation views use =--at-op=@
  --ignore-working-copy= to avoid snapshotting while browsing.
...
- JJ helpers: Add color-preserving jj output helpers for structured buffers
  that need ANSI labels instead of the plain-text ~majutsu-jj-lines~ path.
