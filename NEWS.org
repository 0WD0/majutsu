...
- In ~majutsu-squah~ transient:
  - Add toggle-option for ~-r~.
- In ~majutus-diff~ transient:
...
- Add ~majutsu-annotate~ workflow with chunk navigation, style cycling, and recursive parent tracing.
...
- Highlight JJ description buffers (summary, JJ comments, change list) using git-commit faces.
  ...
  - Allow configurint JJ description comment prefix for comment commands.
...
- Blob buffers now use a wdired-style editable mode:
  ...
  - In Evil normal state, ~i~ enters editable blob mode and switches to insert.
...
- ~majutsu-conflict~ now chooses ~majutsu-conflict-mode~ and ~smerge-mode~
  mutually exclusively when auto-detecting conflict marker style.
- ~majutsu-ediff-resolve~ now runs ~jj resolve~ with 3-way Ediff for 2-sided
  conflicts, and falls back to ~jj diffedit~ for 3+ sided conflicts.
  It also has commit-section DWIM behavior for conflicted file selection.
- ~majutsu-ediff-resolve-with-conflict~ now delegates to
  ~majutsu-ediff-resolve~ for backward compatibility.
- ~majutsu-ediff~ transient no longer mixes resolve actions with range-based
  compare/diffedit arguments; resolve is exposed from dispatcher (~m~).
- ~majutsu-ediff-edit~ now supports non-diff-buffer entry by prompting for a
  changed file and opening the right-side temp file for manual editing.
  Saving finishes the diffedit session by default.
