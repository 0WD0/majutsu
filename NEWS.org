...
- Evil: Keep ~y~ / ~Y~ duplicate bindings on log-family buffers instead of
  generic Majutsu buffers, so diff buffers no longer steal Evil's yank key.
  The Majutsu dispatcher now mirrors Evil top-level key changes such as ~x~,
  ~L~, ~_~, ~*~, ~u~, ~C-r~, and ~`~.
