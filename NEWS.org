...
- Log/Evolog: Add shared graph-entry copy commands under the dispatcher copy
  submenu for section values, rendered fields, hidden entry fields, commit
  hashes, and visible modules.
...
- JJ helpers: Add ~majutsu-jj-buffer-string~ for callers that need full
  ...
  shared ~majutsu-graph-entry~ helpers for graph-prefixed template output,
  hidden metadata, and Magit section rendering.

** Changed

- Bookmarks: The bookmark list buffer now uses a structured =jj bookmark list
  -T= template, preserving jj's own rendering while assigning precise section
