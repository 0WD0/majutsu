...
- Git: Remote management now exposes command-specific options: =remote add=
  supports =--fetch-tags= and =--push-url=, while =remote set-url= supports
  explicit =--fetch= and =--push= URL updates.
