...
- Git: Expose simple upstream options that were missing from the transients:
  ...
  as remote URLs.
- Gerrit: Upload review metadata prompts now reuse Gerrit REST completion
  strategies inspired by =gerrit.el= and =egerrit=. Reviewer/CC prompts query
  active accounts, topic/hashtag prompts reuse open-project metadata, and
  label prompts offer project label votes while filtering to the authenticated
  user's permitted voting range when Gerrit exposes it.
