...
- Bookmark and tag templates must use =primary()= for primary refs ::
  =RefListItem= no longer forwards primary-ref fields directly.
...
- Blob buffers in other windows and frames ::
  Add ~majutsu-find-file-other-window~ and
...
- Operation history and evolution browsing ::
  ~X~ opens operation commands; ~v~ opens evolog.  Majutsu also adds
  ~op log~ / ~op diff~ transients and operation restore/revert actions.

- Interactive revision arrangement ::
  ~majutsu-arrange-mode~ edits =jj arrange=-style graph rewrite plans.
...
- Gerrit review dashboard ::
  The Git dispatcher now exposes the native review dashboard as ~G R~.
...
- Evil dispatcher entries show Evil-facing keys ::
  The dispatcher now displays remapped top-level keys such as ~x~, ~L~, ~_~,
  ~*~, ~u~, ~C-r~, and ~`~.

- Duplicate commands moved out of ~y~ / ~Y~ ::
  Use ~? y~ for ~majutsu-duplicate~.  The separate ~majutsu-duplicate-dwim~
  command was removed; the transient's execute action is now
  ~majutsu-duplicate-execute~.  This leaves Evil yank keys free.
...
- Native jj completion is richer ::
  Majutsu prefers =jj util complete --format json= when available and
  completes full revset expressions.
...
- Log row layouts carry more display metadata ::
  ...
  repeated fields per module, and richer copy/navigation metadata.
...
- ~majutsu-log-field-faces~ was removed ::
  Configure field faces in ~majutsu-log-commit-layout~ with each field's
  ~:face~ property.
