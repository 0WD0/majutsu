...
- The template DSL now supports helper-local ~:bind-self~ in
  ...
  ~self~ when expanding nested template bodies.
...
- ~canonical-log-id~ now uses ~:bind-self~, and log parent id templates now
  prefer explicit ~map + join~ composition over a dedicated implementation
  path.
