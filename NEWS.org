...
- Optional Embark actions ::
  Embark acts on bookmark and workspace completion candidates, bookmark
  and revision sections, and remote headings.  Actions include inspecting
  revisions, tracking remote bookmarks, visiting workspaces, and opening Dired.
  Revision inspection defaults to a diff; editing remains an explicit action.
...
- Bookmark and remote lists ::
  Local bookmarks and remote bookmarks now have separate sections, with
  remote references grouped by remote and =@git= references in their own
  section.  Conflicted targets remain expandable children of each bookmark.
  The remote list command opens this view, including remotes before their
  first fetch; remote headings offer management and fetch actions.
...
- Additional command options ::
  ...
  Revert accepts =--insert-after= and =--insert-before= together.
...
- Evil keybindings ::
  Blob buffers preserve normal text motions and paste: use ~gj~ / ~gk~
  (or ~C-j~ / ~C-k~) for revision navigation, ~gb~ to annotate, ~gf~ to visit
  the workspace file, and ~gr~ to revert the buffer.  The dispatcher shows
  Evil-facing keys; diff context uses ~+~, ~=~, and =~= to add, remove,
  and reset context.
...
- Transient command context ::
  ...
  use the buffer that opened the transient.  Duplicate defaults only to commit
  sections in the region, so file names are not mistaken for revision IDs.
...
- Section folding and refresh ::
  Collapsed sections no longer leave invisible remnants that join adjacent
  headings together.  =jj new= refreshes the buffer it was invoked from.
