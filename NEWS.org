...
- Operations: Add ~majutsu-op-transient~ on ~X~, dedicated ~op log~ / ~op diff~
  transients, a color-preserving structured multiline ~jj op log~ view, expanded
  sectionized ~majutsu-op-show~ / ~majutsu-op-diff~ buffers, graph-preserving
  ~majutsu-evolog~ view, and ~op restore~ / ~op revert~ actions.
  Read-only operation/evolution views use =--at-op=@ --ignore-working-copy= to
  avoid snapshotting while browsing.
...
- Commands: Add Magit-style ad-hoc runners: ~majutsu-command~,
  ...
  Native Evil integration now binds ~|~ to ~majutsu-jj-command~.
...
- Log/Evolog/Operations: Add shared row copy commands under the dispatcher
  copy submenu for section values, rendered fields, hidden entry fields,
  commit hashes, and visible modules.
...
- JJ helpers: Add ~majutsu-jj-buffer-string~ for callers that need full
  plain-text command output instead of line-oriented ~majutsu-jj-lines~, plus
  shared ~majutsu-row~ helpers for graph-prefixed template output, hidden
  metadata, Magit section rendering, and structured copying.
...
- Bookmarks/Templates: Treat =RefListItem= as an explicit group object. It now
  exposes only =primary()= and =tracked_refs()=; bookmark/tag templates must
  access primary-ref fields through =primary()= instead of relying on forwarding
  or direct =self= rendering.
...
- Completion: Standardize history-aware readers and completion categories for
  ...
  workspace names, and repo-relative file paths, and add optional
  Marginalia annotations for Majutsu completion categories. When available,
  Majutsu now prefers the machine-readable =jj util complete= interface for
  native jj completion and falls back to jj's shell completion protocol on
  older jj versions. Native revset prompts now use minibuffer completion at
  point, so expression arguments such as =main | = and =trunk()..ma= keep jj's
  native semantics while still allowing users to move the candidate list into a
  minibuffer for annotation-based filtering. Native jj revset completion now
  includes other workspace working-copy refs in =<workspace>@= form. Revision
  Marginalia payloads are
  cached per minibuffer input context, so annotations remain structured while
  editing revset expressions. The optional Marginalia integration now shows
  richer bookmark, tag, remote, workspace, revision, and file annotations based
  on structured completion payloads.
